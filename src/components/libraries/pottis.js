...
const Pottis = function(targetSVG, startCallback) {
	...
	this.addClick = function(elem, callback) {
		...
		elem.onclick = callback.bind(this, elem);
	}
	...
	this.addDrag = function(handle, target, callback) {
		handle.onmousedown = function(e) {
			...
			_pottis.mousemove = function(e) {
				// let bb = handle.getBBox();
				// let bc = handle.getBoundingClientRect();
				handle.getBBox();
	...
	this.importSVG = function(svgfile, groupid) {
		...
		obj.onload = function() {
			...
			for (let i = 0; i < svgobj.childNodes.length; i++) {
